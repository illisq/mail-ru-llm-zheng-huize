...
model_wrapper = model_wrapper.ModelWrapper() # внутри класса описание 课堂描述
...
def get_text_messages(message):
    ...
    logging.info(f'Request received: Text="{message.text}"')
    if message.text in ['StatLM', 'GPT', 'Llama']:
        print(f'@{message.text}@')
        status, result = model_wrapper.load(message.text, test_inference=True)
        if status:
            bot.send_message(message.from_user.id, "Подгружено") #已加载
        else:
